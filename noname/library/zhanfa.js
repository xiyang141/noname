  ...
  zf_xuyan: {
    ...
    skill: {
      ...
      async content(event, trigger, player) {
        ...
        player.when("chooseToDiscardAfter").filter((evt) => evt == trigger).step(async (event2, trigger2, player2) => {
          if (trigger2.result?.cards?.length) {
            const { cards } = trigger2.result;
            await player2.showCards(cards, `${get.translation(player2)}【火攻】展示的牌`);
          }
        });
      }
    }
