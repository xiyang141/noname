...
class Player extends HTMLDivElement {
  ...
  addExtraEquip(skill, equip, replace = false, preserve) {
    ...
    player2.extraEquip.add(...list);
    player2.$handleEquipChange();
    game.broadcast(
      (player3, list2) => {
        player3.extraEquip.add(...list2);
  ...
  removeExtraEquip(skill, equip = "noequip") {
    ...
    let equips = player2.extraEquip.filter((info) => info[0] != skill);
    if (equip != "noequip") {
  ...
  $handleEquipChange() {
    ...
    cards.forEach((card) => {
      ...
      if (card.extraEquip && !remove) {
        ...
        let preserve = card.extraEquip[2] && !card.extraEquip[2](player2);
        ...
        if (!extraEquips.includes(extra) || preserve) {
          ...
          let remove2 = extraEquip.find((info) => info[1] == extra);
          extraEquip.remove(remove2);
        }
    ...
    extraEquip.forEach((info) => {
      ...
      if (player2.hasEmptySlot(subtype)) {
