          ...
          for (var i in dash4.content) {
            try {
              if (["arenaReady", "content", "prepare", "precontent"].includes(i)) {
                ext[i] = _exports.exec2(`return (${dash4.content[i]});`).return;
                ...
                ext[i] = _exports.exec2(dash4.content[i])[i];
	...
	return ${str} 
          ...
          var callback = () => {
            if (exportext) {
              ...
              if (game.getFileList) {
                game.getFileList("extension/" + page.currentExtension, function(folders, files2) {
                  extension._filelist = files2;
                  proexport();
                });
              } else {
                proexport();
      ...
      var dash2 = (function() {
        ...
        var saveInput = function(view) {
          ...
          try {
            var { card } = _exports.exec2(inputCode);
...
`;
        ...
        var saveInput = function(view) {
          ...
          try {
            var { skill } = _exports.exec2(resultCode);
          ...
          var saveInput = function(view) {
            ...
            try {
              if (["arenaReady", "content", "prepare", "precontent"].includes(link)) {
                var { func: func2 } = _exports.exec2(`func = ${resultCode}`);
                if (typeof func2 != "function") {
                  throw "err";
                }
              } else if (link == "config") {
                var { config } = _exports.exec2(resultCode);
                if (config == null || typeof config != "object") {
                  throw "err";
                }
              } else if (link == "help") {
                var { help } = _exports.exec2(resultCode);
  ...
  (function() {
    ...
    node._initLink = function() {
      ...
      node.update = function() {
        ...
        }).then((response) => response.text()).then(_exports.eval).then(loaded).catch((reason) => {
          console.log(reason);
          delete window.extension;
